...
You are not required to know linux beforehand. BUT a lot of the programs we going to use are only available in linux (or easier to use in linux), so below is a brief how to for linux. It should be enough to get by for the exercises. However, if you get stuck in an exercise because there is a linux command you don't understand or can't get to work then PLEASE just ask :)
...
**Mac:**
You should have [XQuartz](https://www.xquartz.org/) or similar installed so that you can view graphics from the terminal. Else ask when you are here if you have problems.
